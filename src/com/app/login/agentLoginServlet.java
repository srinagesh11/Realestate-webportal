...
public class agentLoginServlet extends HttpServlet {
	...
	static final Logger LOG = LoggerFactory.getLogger(agentLoginServlet.class);
	
	@Resource(name = "jdbc/demo1")
    private DataSource ds;
	Connection conn;
	
   private agentLoginService userValidation =new agentLoginService();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			if(isUserValid)
			{
				...
				LOG.info("doPost : Redirect from agentLoginServlet to TodoServlet");
				 request.getRequestDispatcher("view/agentLogout.jsp").forward(request, response);		
			}
			else
			{
